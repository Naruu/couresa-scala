...
class FunSetSuite {
  ...
  @Test def `filter test`: Unit = {
    new TestSets {
      ...
      val ss = filter(s1, x => x < 0)
      assert(contains(s, 1), "Union 1")
      assert(!contains(ss, 1), "Union 2")
    }
  }

  @Test def `forall test`: Unit = {
    new TestSets {
      forall(x => (x%2 == 0), x => (x%50 == 0))
      assert(!forall(x => (x%2 == 0), x => (x%50 == 0)), "filter1")
      assert(forall(x => (x%50 == 0), x => (x%2 == 0)), "filter2")
    }
  }

  @Test def `map test`: Unit = {
    new TestSets {
      val m = map(x => ((x - 5) * x < 0), x => x+2);
      assert(!m(1), "map1")
      assert(m(3), "map2")
      assert(!m(7), "map2")
      //assert(forall(x => (x%50 == 0), x => (x%2 == 0)), "filter2")
