...
object TimeUsage extends TimeUsageInterface {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
    def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
      val primaryNeeds = columnNames.filter(x => x.startsWith("t01") || x.startsWith("t03") || x.startsWith("t11") || x.startsWith("t1801") || x.startsWith("t1803"))
      val working = columnNames.filter(x => x.startsWith("t05") || x.startsWith("t1805"))
      val notOthers = primaryNeeds ::: working
      (
        primaryNeeds.map(x => col(x)),
        working.map(x => col(x)),
        columnNames.filterNot(x => notOthers.contains(x))
                  .map(x => col(x))
      )
  }
  ...
  /** @return a projection of the initial DataFrame such that all columns containing hours spent on primary needs
    ...
    *   - "elder" otherwise
    * -: sum of all the `primaryNeedsColumns`, in hours
  ...
  /**
    ...
    */
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = timeUsageSummaryDf.map({
    case row => TimeUsageRow(row.getAs[String]("working"), row.getAs[String]("sex"), row.getAs[String]("age"),row.getAs[Double]("primaryNeeds"), row.getAs[Double]("work"), row.getAs[Double]("other"))
  })
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    val queried = summed.groupByKey(row => (row.working, row.sex, row.age))
          ...
          .map({case (key, pn, work, other) => (key._1, key._2, key._3, pn, work, other)})
          .toDF("working", "sex", "age", "primaryNeeds", "work", "other")
          .orderBy("working", "sex", "age").toDF
    timeUsageSummaryTyped(queried)
  }
}
