...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    val new_b = new Boundaries()
    new_b.minX = math.min(boundaries.minX, body.x)
    new_b.minY = math.min(boundaries.minY, body.y)
    new_b.maxX = math.max(boundaries.maxX, body.x)
    new_b.maxY = math.max(boundaries.maxY, body.y)
    new_b
  }
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val new_b = new Boundaries()
    new_b.minX = math.min(a.minX, b.minX)
    new_b.minY = math.min(a.minY, b.minY)
    new_b.maxX = math.max(a.maxX, b.maxX)
    new_b.maxY = math.max(a.maxY, b.maxY)
    new_b
  }
