...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    val seqs = points.groupBy(p => findClosest(p, means))
    if (means.isEmpty) Map[Point, Seq[Point]]() else means.map(m => m -> seqs.getOrElse(m, Seq[Point]()) ).toMap
  }

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    val seqs = points.groupBy(p => findClosest(p, means)).par
    if (means.isEmpty) ParMap[Point, ParSeq[Point]]() else means.map(m => m -> seqs.getOrElse(m, ParSeq[Point]()) ).toMap
  }
