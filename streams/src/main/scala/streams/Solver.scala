...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    /*
    def appendNeighbors(neighbors: List[(Block, Move)], history: List[Move]): LazyList[(Block, List[Move])] =
        if(neighbors.isEmpty) LazyList()
        else (neighbors.head._1 , neighbors.head._2 :: history) #:: appendNeighbors(neighbors.tail, history)
    appendNeighbors(b.legalNeighbors, history)
    */
    b.legalNeighbors.view.map(element => {
      (element._1, element._2 :: history)
      }).to(LazyList)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = {
            if(initial.isEmpty) LazyList()
              ...
              val newNeighbors: LazyList[(Block, List[Move])] = initial.flatMap(element => neighborsWithHistory(element._1, element._2))
              val validNew = newNeighborsOnly(newNeighbors, explored)
              val result = initial ++ from(validNew, explored ++ validNew.map(_._1).toSet)
              result
            }
           }
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter(element => done(element._1))
