...
object ParallelCountChange extends ParallelCountChangeInterface {
  ...
  def countChange(money: Int, coins: List[Int]): Int =  {
    def countWithCoins(money:Int, coins: List[Int]): Int = coins match {
    case Nil => 0
    case head :: tail => {
      if (head <= money) countChange(money - head, coins) + countChange(money, tail)
    ...
    else countWithCoins(money, coins)
  ...
  def moneyThreshold(startingMoney: Int): Threshold = (sum: Int, L: List[Int]) => sum <= (2 * startingMoney / 3)
    

  /** Threshold heuristic based on the total number of initial coins. */
  def totalCoinsThreshold(totalCoins: Int): Threshold = (sum: Int, L: List[Int]) => L.length <= (2 * totalCoins / 3)


  /** Threshold heuristic based on the starting money and the initial list of coins. */
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
     (sum: Int, L: List[Int]) => (L.length * sum) <= (startingMoney * allCoins.length / 2)
  }
}
